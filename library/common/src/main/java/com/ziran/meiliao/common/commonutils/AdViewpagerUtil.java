...
public class AdViewpagerUtil {
   ...
   public List mDatas;
    ...
    public void setDataAndRef (List<?> datas, String[] url) {
        mDatas = datas;
        urls = url;
        initVps();
    }
...
//            imageView.setBorderRadius(0);
        ...
        if (urls.length > 0) {
            ...
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    ...
                    ImageLoaderUtils.displayTager(context, mImageViews[i + 1], url, R.mipmap.ic_loading_square_big);
                    mImageViews[i + 1].setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick (View v) {
                            // TODO Auto-generated method stub
                            if (onAdItemClickListener != null) {
                                onAdItemClickListener.onViewPagerItemClick(mImageViews[index + 1], index, url);
                            }
                        }
                    });
                }
            }
            ImageLoaderUtils.displayTager(context, mImageViews[0], urls[urls.length - 1], R.mipmap.ic_loading_square_big);
            ImageLoaderUtils.displayTager(context, mImageViews[length - 1], urls[0], R.mipmap.ic_loading_square_big);
        }
    }
     ...
     */
    public void initVps () {
        if (EmptyUtils.isEmpty(urls)) return;
        initAdimgs(urls);
    ...
    public interface OnAdItemClickListener {
        void onViewPagerItemClick (View clickView, int position, String url);
    }
