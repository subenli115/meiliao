...
import cn.rongcloud.live.controller.MyUserBean;
...
public class LiveKit {
     ...
     * 当前登录用户id
     */
    private static MyUserBean currentUser;
     ...
     * @param user 当前用户
     */
    public static void setCurrentUser(MyUserBean user) {
        currentUser = user;
    }
     ...
     * 设置当前登录用户，通常由注册生成，通过应用服务器来返回。
     */
    public static void setCurrentUser(String id, String userName, String userHead) {
        setIsConnect(true);
        currentUser = new MyUserBean(id, userName, userHead);
    }
     ...
     * @param callback 连接回调
     */
    public static synchronized void connect(String token, final RongIMClient.ConnectCallback callback) {
    ...
    public static void quitChatRoom(final RongIMClient.OperationCallback callback) {
        if (TextUtils.isEmpty(currentRoomId)) return;
...
//            throw new RuntimeException("currentUser should not be null.");
        ...
        final Message msg = Message.obtain(currentRoomId, Conversation.ConversationType.CHATROOM, msgContent);
        RongIMClient.getInstance().sendMessage(msg, null, null, new RongIMClient.SendMessageCallback() {
            ...
            public void onError(Integer integer, RongIMClient.ErrorCode errorCode) {
                Log.d(TAG, "onError: " + integer + "/" + errorCode.getMessage() + "/" + errorCode.getValue());
    ...
    public static void setCurrentRoomId(String currentRoomId) {
        LiveKit.currentRoomId = currentRoomId;
    }


    public static TextMessage getNotifytionMessage(){
        TextMessage textMessage = TextMessage.obtain("直播消息：我们提倡绿色直播，严禁恶意攻击他人，包括谩骂、诽谤、地域攻击等。");
        textMessage.setExtra("NOTIFYCATION");
        textMessage.setUserInfo(LiveKit.getCurrentUser());
        return textMessage;
    }
}
