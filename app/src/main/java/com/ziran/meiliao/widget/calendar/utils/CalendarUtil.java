package com.ziran.meiliao.widget.calendar.utils;
...
public class CalendarUtil {
    ...
    private static int leapDays(int year) {
        if (leapMonth(year) != 0) {
            if ((LUNAR_INFO[year - 1900] & 0x10000) != 0) return 30;
            else return 29;
        } else return 0;
    }
    ...
    private static int monthDays(int year, int month) {
        if ((LUNAR_INFO[year - 1900] & (0x10000 >> month)) == 0) return 29;
        else return 30;
    }
    ...
    public CalendarUtil(Calendar cal) {
        ...
        for (iMonth = 1; iMonth < 13 && offset > 0; iMonth++) {
            ...
            } else daysOfMonth = monthDays(mLuchYear, iMonth);

            offset -= daysOfMonth;
            // 解除闰月
            if (isLoap && iMonth == (leapMonth + 1)) isLoap = false;
            if (!isLoap) {
            }
        }
    ...
    public static String getChinaDayString(int day) {
        ...
        if (day > 30) return "";
        if (day == 10) return "初十";
        else return chineseTen[day / 10] + CHINESE_NUMBER[n];
    }
