...
import com.ziran.meiliao.widget.calendar.theme.ADCircleDayTheme;
...
//        }else  if(day== currDay  && currDay != selDay   && currMonth == selMonth){//今日绘制圆环
    ...
    protected void drawDecor(Canvas canvas, int column, int row, int year, int month, int day) {
        if(calendarInfos != null && calendarInfos.size() >0){
            if(!isCalendarInfoNew(year,month,day))return;
