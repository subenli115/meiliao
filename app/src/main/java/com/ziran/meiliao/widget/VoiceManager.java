...
public class VoiceManager {
    ...
    public ArrayList<File> getmRecList() {
        return mRecList;
    }

    public void setmRecList(ArrayList<File> mRecList) {
    ...
    private void startPlay(boolean init) {
        try {
            ...
            if (prepareMedia(mMediaPlayer, playFilePath)) {
                ...
                String voiceLength = String.format("%02d:%02d",
                        ts.mSpanMinute, ts.mSpanSecond);
                //播放进度回调
                if (voicePlayCallBack != null) {
...
//                }
     ...
     * 继续或暂停播放
     */
    public void continueOrPausePlay(String FilePath) {

        Log.e("mDeviceState",""+mDeviceState);
        if (mDeviceState == MEDIA_STATE_PLAY_DOING) {
            ...
            //播放
            Log.e("mDeviceState1",""+mDeviceState);
     ...
     * @return
     */
    public boolean stopRecorder(MediaRecorder mr, boolean release) {
        boolean result = false;
        try {
            if (mr != null) {
                mr.stop();
