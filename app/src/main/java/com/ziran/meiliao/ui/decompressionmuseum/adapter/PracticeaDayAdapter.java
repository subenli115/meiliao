...
public class PracticeaDayAdapter extends RecyclerView.Adapter<PracticeaDayAdapter.ViewHolder> {

    private final int mIsGif;
    private  Activity mActivity;
    private  TextView arlPunch;
    private  RotateAnimation rotateAnimation;
    private List<AlbumPracBean.DataBean.PunchBean>  data;
    ...
    public PracticeaDayAdapter(List<AlbumPracBean.DataBean.PunchBean> data, Context context, int isGif, Activity activity) {
        this.mIsGif=isGif;
        this.data = data;
...
//    }
    ...
    @Override
    public PracticeaDayAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        // 实例化展示的view
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_gift_week_view, parent, false);
        // 实例化viewholder

        PracticeaDayAdapter.ViewHolder viewHolder = new PracticeaDayAdapter.ViewHolder(v);
    ...
    @Override
    public void onBindViewHolder(final PracticeaDayAdapter.ViewHolder holder, final int position) {
        // 绑定数据
        AlbumPracBean.DataBean.PunchBean punchBean = data.get(position);
        ViewGroup.LayoutParams layoutParams = holder.itemView.getLayoutParams();
        int width = mActivity.getWindowManager().getDefaultDisplay().getWidth();
        layoutParams.width = (width - dip2px(context, 22)) / 9;//
        holder.itemView.setLayoutParams(layoutParams);
        holder.tvDay.setText(punchBean.getDays()+"");
        Glide.with(context).load(punchBean.getPicture()).transform(new GlideRoundTransform(context)).into(holder.tvOutSide);
        holder.tvOutSide.setVisibility(View.VISIBLE);

        if(mIsGif==1){
            holder.tvOutSide.setVisibility(View.VISIBLE);
            holder.tvOutSide.setAnimation(shakeAnimation(4));
            //动画监听
