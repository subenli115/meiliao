 ...
 *  音频练习 on 2018/8/20.
 */
public class FitnessPlayActivity extends CommonHttpActivity<PracticeOneDataPresenter, CommonModel> implements View.OnClickListener, MediaPlayer.OnBufferingUpdateListener {
    @Bind(R.id.ntb_title)
    ...
    public static void startAction(Context mContext, FitnessSavePracticeBean.DataBean.MusicsBean musicsBean) {
        Intent intent = new Intent(mContext, FitnessPlayActivity.class);
        intent.putExtra("musicsBean",musicsBean);
    ...
    public void initView() {
        ...
        if(getIntent()!=null){
            Intent intent = getIntent();
             musicsBean = (FitnessSavePracticeBean.DataBean.MusicsBean)intent.getSerializableExtra("musicsBean");
        }
        ...
        Glide.with(mContext).load("https://www.dgli.net/resource/images/practiceActivity/1bg.png").error(R.mipmap.bg_jdx_nine_dot).into(ivbg);
        url = musicsBean.getLoadUrl();
        initMediaPlayer(url);
//        ntbTitle.setTitleText(musicsBean.getName());
        ImageLoaderUtils.displayTager(mContext, ivJygAlbumPlayCover, musicsBean.getImage(), R.mipmap.ic_jyg_share_music_norm);
        initAnimation();
        playMusic();
