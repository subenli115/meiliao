...
/**
 * Created by 音频练习 on 2018/8/20.
 */
public class EmotionPlayerActicity extends CommonHttpActivity<PracticeOneDataPresenter, CommonModel> implements PracticeOneDataContract.View, View.OnClickListener, MediaPlayer.OnBufferingUpdateListener {
    ...
    public static void startAction(Context mContext,String url,String title) {
        Intent intent = new Intent(mContext, EmotionPlayerActicity.class);
        mUrl=url;
        mTitle=title;
...
//            ivJygAlbumPlayCover.startAnimation(rotate);
    ...
    private void startPlayMusic(String url,String title) {
//        Glide.with(mContext).load(result.getBgPic()).error(R.mipmap.jdx_bg_live).into(ivbg);
//        url = result.getUrl();
        initMediaPlayer(url);
        ntbTitle.setTitleText(title);
//        ImageLoaderUtils.displayTager(mContext, ivJygAlbumPlayCover, "", R.mipmap.ic_jyg_share_music_norm);
        playMusic();
    }
